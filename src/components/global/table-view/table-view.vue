  ...
  <div class="m-table">
    ...
    >
      ...
      >
        ...
        <template slot-scope="scope">
          <ex-slot v-if="tHeaderItem.render" :render="tHeaderItem.render" :row="scope.row" :index="scope.$index" :column="tHeaderItem" />
          <!-- 需要格式化的数据结构 -->
          <span v-else-if="tHeaderItem.format">{{ scope.row[tHeaderItem.prop] | formatters(tHeaderItem.format) }}</span>
          <!-- 纯展示数据 -->
          ...
          >
            ...
            >
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item
                  v-for="(item, index) in operateObj.list.slice(2)"
                  v-if="isActionShow(item.isShow, scope.row)"
                  :key="item.id"
                  @click.native="() => {
...
export default {
    ...
    checkedRadio: {
      ...
      validator(obj) {
        if (!Object.keys(obj).length) {
          console.error('checkedRadio不能为空对象且isRidio为true')
        }
        return true
      },
    }
  ...
  computed: {
    ...
    headerCellStyle(){
      return {
        ...
        fontWeight: '400',
        backgroundColor: this.$store.state.settings.tableStyle.tableHeaderBackgroundColor,
  ...
  methods: {
    ...
    // 单选table需要回显选中的数据
    showRadioTableData() {
      if(this.isRidio && this.tableData.length && this.checkedRadioTurn && this.checkedRadio &&
...
<style scoped="scoped" lang="scss">
  .m-table {
    ...
    ::v-deep .el-table {
      td.is-right, th.is-right {
        text-align: right !important;
      }

      td.is-left, th.is-left {
        text-align: left !important;
      }

      td.is-center, th.is-center {
